...
export class HumanizeCurrencyPipe implements PipeTransform {
  ...
  transform(value: number): string {
    ...
    return this.formatValueWuthSuffix(splittedValue);

  }

  private formatValueWuthSuffix(splittedValue: string[]) {
  ...
  getCurrencySymbol() {
    const { value } =
      new Intl.NumberFormat(this.localeId, { style: 'currency', currency: this.currencyCode })
